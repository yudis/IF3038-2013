...
public class GetAll extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        URLConnection connection = new URL("http://tubes4progin.ap01.aws.af.cm/GetAll").openConnection();
        ...
        if(status == 200) {
            try {
                ...
                NodeList somethings = doc.getElementsByTagName("something");
                if(somethings != null) { 
                    for(int i = 0; i < somethings.getLength(); i++) {
                        out.print(somethings.item(i).getFirstChild().getNodeValue() + '\n');
                    }
                }
            }
            catch(Exception e) {
