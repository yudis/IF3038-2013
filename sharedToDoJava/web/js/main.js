...
function validate_email_exist(str)
{
    ...
    xmlhttp.onreadystatechange = function()
    {
        ...
        if(xmlhttp.readyState==4 && xmlhttp.status==200)
            document.getElementById("error_email_exist").innerHTML = json.response;
        
        if(json.response_register == "")
            return true;
        else
            return false;
    };
    
    xmlhttp.open("GET", "jsp/register_validation.jsp?p="+str, true);
    xmlhttp.send();
}
...
function validate_username_exist(str)
{
    ...
    {    	    
        if(xmlhttp.readyState==4 && xmlhttp.status==200) {
        	var json = eval('('+xmlhttp.responseText+')');
        	document.getElementById("error_username_exist").innerHTML = json.response;
    ...
    xmlhttp.open("GET", "jsp/register_validation.jsp?q="+str, true);
    xmlhttp.send();
}
...
//    alert("masuk");
    ...
    {    	
        if(xmlhttp.readyState==4 && xmlhttp.status==200) {        	
        	var json = eval('('+xmlhttp.responseText+')');
            document.getElementById("error_signin").innerHTML = json.response;            
            if(json.response == "") {
    ...
    xmlhttp.open("GET", "jsp/login.jsp?u="+usrnm+"&p="+psswrd, true);
    xmlhttp.send();
}
