...
function SearchmodeHeader(){
	...
	window.location="search.jsp?m="+mode+"&s="+string;
}
...
function Search(s,m){
	...
	xmlhttp.onreadystatechange=function()
	  ...
	  if (xmlhttp.readyState==4 && xmlhttp.status==200)
        ...
        xmlhttp.open("GET","search?oo="+outside_open+"&o="+open+"&c="+close+"&m="+mode+"&s="+string,true);
	xmlhttp.send();
}
...
function Searchbypage(s,m,page){
	...
	xmlhttp.open("GET","search?oo="+outside_open+"&o="+open+"&c="+close+"&m="+mode+"&s="+string+"&page="+page,true);
	xmlhttp.send();
}
