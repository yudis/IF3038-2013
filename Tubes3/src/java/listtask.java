				...
				if (cat.equals("all")) {
					...
					while (rs.next()) {
						...
						while (rs2.next()) {
							...
							out.println("<div><a href='tugas.jsp?id="+rs2.getString("task_id")+"'>"+rs2.getString("task_name")+"</a></div><div>Deadline: <strong>"+rs2.getString("deadline")+"</strong></div><div>Status: <strong>"+status+"</strong></div><div>Ubah Status: <button id='editStatus' onclick='EditStatus(1,\""+rs2.getString("task_id")+"\")'>Selesai</button> <button id='editStatus' onclick='EditStatus(0,\""+rs2.getString("task_id")+"\")'>Belum</button></div><br/><div>Tags:<ul class='tag'>");
