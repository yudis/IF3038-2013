Rp(function() {
	...
	genTask = function(task) {
		...
		title.setAttribute('href','view_tugas.jsp?task_id='+task.task_id);
	...
	genCategory = function(category) {
		...
		title.setAttribute('href','dashboard.jsp');
		label.appendChild(title);
		h1.appendChild(label);
		header.appendChild(h1);
	...
	genUser = function(user) {
		...
		title.setAttribute('href','profile.jsp?user_id='+user.user_id);
		label.appendChild(span);
		label.appendChild(title);
		h1.appendChild(label);
		header.appendChild(h1);
	...
	Rp('#searchBar').on('keyup', function() {
		...
		xmlhttp.open("GET","core/search?q="+q+"&mode="+mode+"&autocomplete=1",true);
		xmlhttp.send();
	});
	...
	assignee_autocomplete = function(field) {
		...
		xmlhttp.open("GET","core/search?q="+q+"&mode=5&autocomplete=1",true);
		xmlhttp.send();
	};
	...
	tag_autocomplete = function(field) {
		...
		xmlhttp.open("GET","core/search?q="+q+"&mode=6&autocomplete=1",true);
		xmlhttp.send();
	};
	...
	Rp('#commentForm').on('submit', function(e) {
		...
		xmlhttp=new XMLHttpRequest();
		xmlhttp.open("POST","core/postComment",false);
	...
	Rp('#loginForm').on('submit', function(e) {
		...
		xmlhttp=new XMLHttpRequest(); 
		xmlhttp.open("GET","core/auth?username="+u+"&password="+p,false);
		xmlhttp.send();
		var parsedJSON = eval('('+xmlhttp.responseText+')');
...
function updateTask(mode,task_id){
	...
	xmlhttp=new XMLHttpRequest();
	xmlhttp.open("POST","core/updateTask",false);
	xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
...
function updateProfile(mode,user_id){
        ...
        xmlhttp=new XMLHttpRequest();
        xmlhttp.open("POST","core/updateProfile",false);
        xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
...
function negateTask(task_id){
	xmlhttp=new XMLHttpRequest();
	xmlhttp.open("POST","core/negateTask",false);
	xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
...
function deleteComment(comment_id){
	xmlhttp=new XMLHttpRequest();
	xmlhttp.open("POST","core/deleteComment",false);
	xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
...
function deleteTask(task_id){
	xmlhttp=new XMLHttpRequest();
	xmlhttp.open("POST","core/deleteTask",false);
	xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	xmlhttp.send("task_id="+task_id);
...
function deleteCategory(category_id){
	xmlhttp=new XMLHttpRequest();
	xmlhttp.open("POST","core/deleteCategory",false);
	xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
...
function refreshComment(task_id,page){
	_page = page;
	xmlhttp=new XMLHttpRequest();
	xmlhttp.open("GET","core/getComment?task_id="+task_id+"&offset="+((page-1)*10),false);
	xmlhttp.send();
	var parsedJSON = eval('('+xmlhttp.responseText+')');
	...
	xmlhttp=new XMLHttpRequest();
	xmlhttp.open("GET","core/countComment?task_id="+task_id,false);
	xmlhttp.send();
...
function refreshTask(user_id,category_id){
	...
	xmlhttp=new XMLHttpRequest();
	xmlhttp.open("GET","core/getTask?user_id="+user_id+"&category_id="+category_id,false);
	xmlhttp.send();
	var parsedJSON = eval('('+xmlhttp.responseText+')');
	header = document.createElement('header');
...
function refreshCategory(user_id){
	xmlhttp=new XMLHttpRequest();
	xmlhttp.open("GET","core/getCategory?user_id="+user_id,false);
	xmlhttp.send();
	var parsedJSON = eval('('+xmlhttp.responseText+')');
...
function selectCategory(user_id,category_id) {
	...
	document.getElementById('taskLink').setAttribute('href','new_tugas.jsp?category_id='+category_id);
...
function getSearchResult(q,mode,page){
	_page = page+1;
	xmlhttp=new XMLHttpRequest();
	xmlhttp.open("GET","core/search?q="+q+"&mode="+mode+"&autocomplete=0&offset="+page*10,false);
	xmlhttp.send();
	var parsedJSON = eval('('+xmlhttp.responseText+')');
	for (index=0; index < parsedJSON.length; index++) {
