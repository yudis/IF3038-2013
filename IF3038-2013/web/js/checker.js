Rp(function() {
	Rp('#email').on('keyup', function() {
		...
		xmlhttp.open("GET","core/search?q="+this.value+"&mode=8&equals=1",false);
		xmlhttp.send();
		var parsedJSON = eval('('+xmlhttp.responseText+')');
		if (match&&(this.value!=Rp('#password').val())&&typeof parsedJSON.email === "undefined") {
			// bener
	...
	Rp('#username').on('keyup', function() {
		...
		xmlhttp=new XMLHttpRequest();
		xmlhttp.open("GET","core/search?q="+this.value+"&mode=7&equals=1",false);
		xmlhttp.send();
		var parsedJSON = eval('('+xmlhttp.responseText+')');
		if ((this.value != Rp('#email').val())&&match&&typeof parsedJSON.username === "undefined"){
			//valid
			Rp(this).removeClass('invalid');
			_username = true;
		}else{
