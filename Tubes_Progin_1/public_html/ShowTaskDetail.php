...
while ($row = mysqli_fetch_array($result)) {
    ...
    while ($rowAttach = mysqli_fetch_array($resultAttachment)) {
        ...
        if ((substr_compare($attachExt, "jpg", 0)) or (substr_compare($attachExt, "jpeg", 0)) or
                (substr_compare($attachExt, "png", 0))) {   // if image
            echo "<img src='" . $rowAttach['path'] . "' height='100' width='100'";
        } else if (substr_compare($attachExt, "mp4", 0)) {  // if video
    ...
    echo "<form>";
    echo "<textarea id='addCommentaryTask" . $row['taskID'] . "'></textarea>";
    echo "<button onclick='submitNewCommentary('" . $_SESSION['username'] . "', '" . $row['taskID'] . "');'>Submit commentary</button>";
    echo "</form>";
