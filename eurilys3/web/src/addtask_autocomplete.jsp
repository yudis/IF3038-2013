...
<%  
    ...
    try {
        ...
        System.out.println("Berhasil connect ke Mysql JDBC Driver - addtask_autocomplete.jsp ");
    } catch (ClassNotFoundException ex) {
        System.out.println("Where is your MySQL JDBC Driver? - addtask_autocomplete.jsp");
    }
    ...
    if (hint.length() > 0) {
        for (int i=0; i<idList.size(); i++) {
            if (buffer.equals("")) {
                buffer = "<div class='search_recommend' id='addtask_ass_"+idList.get(i)+"' onclick=\"javascript:addAddTaskAssigne('"+idList.get(i)+"');\">"+idList.get(i)+"</div>";
            }
            else {
                buffer = buffer + "<br> <div class='search_recommend' id='addtask_ass_"+idList.get(i)+"' onclick=\"javascript:addAddTaskAssigne('"+idList.get(i)+"');\">"+idList.get(i)+"</div>";
            }
        }
    }
